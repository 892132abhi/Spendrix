...
const CreateJobPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const errorMessage = error.response?.data?.detail || "Failed to create job. Check inputs.";
    ...
    <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-20 font-sans">
      ...
      <header className="flex items-center justify-between text-left">
        <div>
          ...
          <h1 className="text-3xl font-black text-slate-800 tracking-tight">Post a New Position</h1>
        </div>
      </header>

      <form onSubmit={handleSubmit} className="bg-white/70 backdrop-blur-xl border border-white rounded-[3rem] p-10 shadow-2xl shadow-slate-200/50 space-y-10 text-left">
        
        <section className="space-y-6">
          <h3 className="text-[10px] font-black text-indigo-500 uppercase tracking-[0.3em]">Basic Information</h3>
        ...
        <section className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="md:col-span-2">
              <label className="block text-xs font-black text-slate-700 uppercase tracking-widest mb-2">Required Qualification</label>
              <input 
                name="Qualification"
        ...
        <section className="space-y-4">
          <h3 className="text-[10px] font-black text-indigo-500 uppercase tracking-[0.3em]">Role Description</h3>
          <textarea 
            name="description"
