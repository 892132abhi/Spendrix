...
import { 
  FiArrowLeft, FiUser, FiBriefcase, FiPhone, 
  FiTag, FiLinkedin, FiLayers, FiFileText, FiSave, FiX, FiCheck 
} from 'react-icons/fi';
...
const InterviewerEditprofile = () => {
  ...
  if (loading) return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-slate-50/50">
      ...
      <div className="animate-pulse text-slate-405 font-black uppercase tracking-widest text-xs">Syncing Identity Records...</div>
    </div>
  );
    ...
    <div className="min-h-screen bg-slate-50/50 py-12 px-6 font-sans">
      <div className="max-w-3xl mx-auto bg-white rounded-3xl border border-slate-200/60 shadow-xl overflow-hidden">
        ...
        <header className="bg-gradient-to-r from-slate-950 via-slate-900 to-indigo-950 p-10 text-white relative">
          ...
          <p className="text-slate-400 text-xs mt-1 uppercase tracking-wider font-semibold">Update your professional profile credentials and system metrics.</p>
        </header>
        ...
        <form onSubmit={handleSubmit} className="p-8 md:p-10 space-y-8 text-left">
          ...
          <div className="flex flex-col sm:flex-row items-center gap-6 pb-8 border-b border-slate-100">
            <div className="w-24 h-24 rounded-3xl bg-slate-100 overflow-hidden border-4 border-white shadow-md relative shrink-0">
              {preview ? (
                <img src={preview} alt="Preview" className="w-full h-full object-cover animate-in fade-in-50" />
              ) : (
            ...
            <div className="space-y-2.5 text-center sm:text-left">
              ...
              <div className="relative inline-block cursor-pointer">
                <input 
                  ...
                  className="text-xs font-semibold text-indigo-650 file:mr-4 file:py-2.5 file:px-4.5 file:rounded-xl file:border-0 file:bg-indigo-50 file:text-indigo-650 hover:file:bg-indigo-100 cursor-pointer transition-colors" 
                />
              </div>
            </div>
          </div>
          ...
          <div className="space-y-3">
            ...
            <div className="flex flex-wrap gap-2 pt-1">
              {formData.skills.map((skill, index) => (
                <span key={index} className="inline-flex items-center gap-1.5 px-3.5 py-1.5 bg-indigo-50 border border-indigo-100 text-indigo-755 text-[10px] font-black rounded-full shadow-sm">
                  {skill.toUpperCase()}
          ...
          {/* RESUME UPLOADER */}
          <div className="p-6 bg-slate-905 rounded-3xl text-white flex flex-col md:flex-row justify-between items-center gap-6 border border-slate-800 bg-slate-900">
            ...
            <div className="relative">
              ...
              />
              {resume && (
                <span className="absolute -bottom-6 left-0 text-[9px] font-black text-emerald-450 flex items-center gap-1">
                  <FiCheck className="w-3 h-3" /> Selected: {resume.name.length > 20 ? `${resume.name.substring(0, 17)}...` : resume.name}
                </span>
              )}
            </div>
          </div>
          ...
          <div className="flex justify-end items-center gap-6 pt-6 border-t border-slate-100">
            <button 
              type="button" 
              onClick={() => navigate(-1)} 
              className="text-[10px] font-black uppercase text-slate-400 hover:text-rose-505 transition-colors tracking-widest cursor-pointer bg-transparent border-0 outline-none hover:text-rose-500"
            >
              Discard Changes
            </button>
            <button 
...
const InputGroup = ({ label, name, value, onChange, type = "text", icon }) => (
  <div className="space-y-2">
    <label className="block text-[10px] font-black text-slate-450 uppercase tracking-wider ml-1">{label}</label>
    <div className="relative">
